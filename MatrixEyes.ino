// Trinket/Gemma + LED matrix backpack jewelry.  Plays animated
// sequence on LED matrix.  Press reset button to display again,
// or add optional momentary button between pin #1 and +V.

// THERE IS NO ANIMATION DATA IN THIS SOURCE FILE, you should
// rarely need to change anything here.  EDIT anim.h INSTEAD.

#define BRIGHTNESS   12 // 0=min, 15=max
#define I2C_ADDR   0x70 // Edit if backpack A0/A1 jumpers set

#include <Wire.h>
#include <avr/power.h>
#include <avr/sleep.h>
#include "anim.h"       // Animation data is located here



static const uint8_t PROGMEM reorder[] = { // Column-reordering table
     };

void ledCmd(uint8_t x) { // Issue command to LED backback driver
  Wire.beginTransmission(I2C_ADDR);
  Wire.write(x);
  Wire.endTransmission();
}

void clear(void) { // Clear display buffer
  Wire.beginTransmission(I2C_ADDR);
  for(uint8_t i=0; i<17; i++) Wire.write(0);
  Wire.endTransmission();
}

void setup() {

  power_timer1_disable();    // Disable unused peripherals
  power_adc_disable();       // to save power
#if MIN_REPS > 0
  PCMSK |= _BV(PCINT1);      // Set change mask for pin 1
#endif
  Wire.begin();              // I2C init
  clear();                   // Blank display
  ledCmd(0x21);              // Turn on oscillator
  ledCmd(0xE0 | BRIGHTNESS); // Set brightness
  ledCmd(0x81);              // Display on, no blink
}

uint8_t rep = MAX_REPS;

uint8_t sprite[sizeof(spritemap)];
uint8_t frameno = 0;
uint8_t i,spritex,spritey,spritemove=20,spritepos=sizeof(spritemap);

void loop() {
  //position the sprite
  if(!spritemove--) {
    spritemove = random(50,120);
    spritepos = random(SPRITE_LEFT,SPRITE_RIGHT);
    //origin is lower left; 1nd quadrant.
    spritey = 8-(spritepos+random(0,SPRITE_UP));
    spritex = spritepos;
  }

  //load the sprite, shifted by x and y
  for(uint8_t j=0; j<sizeof(spritemap); j++) {    // 8 rows...
    sprite[j]=pgm_read_byte(&spritemap[(j+spritey) % sizeof(spritemap)])>>spritex;
    //we move the sprite up by adding spritey to the starting row
    //and right by shifting bits right by spritex
    }

  i = 0; //draw frame 0 by default
  if (frameno<sizeof(anim)/8) { // if frame counter in valid range
    //allowing frameno to be outside the valid range, allows us to use it as a timer between blinks.
    i = frameno*8; //draw with the current frame, each frame has 8 rows = 8 bytes
    }
  Wire.beginTransmission(I2C_ADDR);
  Wire.write(0);                  // Start address
  for(uint8_t j=0; j<8; j++) {    // 8 rows per frame
    Wire.write(
      pgm_read_byte(&reorder[
        pgm_read_byte(&anim[i++])
        &~ sprite[j] //change &~ to | for positive sprite on dark background
        ])
      );
    Wire.write(0);
    }
  Wire.endTransmission();

  delay(FRAME_DELAY*5);
  if(!frameno--) {
    frameno = random(sizeof(anim)/8, 180);
    }

#if MIN_REPS > 0
    if((7==frameno)          // If eye closed 
        && !--rep) {         // and if last cycle...
      ledCmd(0x20);          // LED matrix in standby mode
      GIMSK = _BV(PCIE);     // Enable pin change interrupt
      power_all_disable();   // All peripherals off
      set_sleep_mode(SLEEP_MODE_PWR_DOWN);
      sleep_enable();
      sei();                 // Keep interrupts disabled
      sleep_mode();          // Power down CPU (pin 1 will wake)
      // Execution resumes here on wake.
      GIMSK = 0;             // Disable pin change interrupt
      rep   = random(MIN_REPS, MAX_REPS);// Reset animation counter
      power_timer0_enable(); // Re-enable timer
      power_usi_enable();    // Re-enable USI
      Wire.begin();          // Re-init I2C
      clear();               // Blank display
      ledCmd(0x21);          // Re-enable matrix
    }
#endif

}

ISR(PCINT0_vect) {} // Button tap


